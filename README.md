...
- **Спутники планет:** конфигурации галилеевых спутников, их прохождения по
  диску, тени, покрытия и затмения; взаимные покрытия и затмения спутников
  друг другом (сезон 2026–2027); положение Титана с расстоянием до края диска
  Сатурна.
- **Малые тела:** кометы с кросс-матчем по каталогам, перигелий, минимум
  расстояния и максимум блеска; яркие астероиды. Блеск комет берётся из
  наблюдений COBS, а не из формулы по архивным параметрам: для комет,
  сменивших активность, она ошибается на величины.
...
Пост собирается с учётом лимита Telegram в 4096 знаков: длина считается в тех
же единицах UTF-16, что и у самого мессенджера, а разрыв между частями никогда
не проходит посреди события.

Галочка **«Значки событий вместо ▪️»** в предпросмотре заменяет общий маркер
строки значком: планетам свои символы, метеорам, кометам, покрытиям и пускам
свои. Пост на полсотни строк перестаёт читаться как сплошная стена. В
командной строке то же самое даёт флаг `--icons`. Весь набор символов собран в
одном модуле — поменять его под свою редакцию несложно.
...
Тот же редактор выпуска в браузере, с входом по логину и паролю:
**https://astrocalendar.optegra.ru**

Расчёт месяца идёт на сервере в фоне с показом прогресса, результат
сохраняется и открывается мгновенно. Есть отбор событий галочками, правка
формулировок, фильтры, готовый пост с разбиением по лимиту Telegram, отчёт
проверки и выгрузка файлами. Карты и отправка в канал остаются в настольной
версии.

Сайт закрыт целиком: без входа не отдаётся ничего. Пароли хранятся как
scrypt-хэш с индивидуальной солью, после пяти неудачных попыток вход
закрывается на четверть часа.

Развёртывание, управление пользователями и обновление — в
[`docs/web.md`](docs/web.md). Локальный запуск:
...
**Блеск комет — только по наблюдениям.** Формула MPC `m = g + 5·lg Δ + k·lg r`
опирается на два архивных параметра и для комет, изменивших активность, врёт
на величины: 65P/Gunn по ней выходит +10,5ᵐ при наблюдаемых 18,8ᵐ. Поэтому
модель отвечает только за форму кривой блеска, а её уровень калибруется по
последнему наблюдению из COBS. Комета, которой нет ни в COBS, ни в таблице
ван Бёйтенена, в календарь не идёт вовсе: её яркость ничем не подтверждена.

**День или ночь — по регионам.** Полоса покрытия тянется на тысячи километров,
и пока в Якутии полдень, над Новой Землёй может быть глубокая ночь. Высота
Солнца считается для каждого региона полосы отдельно, с четырьмя градациями:
ночь, сумерки, светлые сумерки, день.
...
| Кометы: орбиты | MPC CometEls.txt | орбитальные элементы |
| Кометы: блеск | COBS, astro.vanbuitenen.nl | наблюдения вместо модели |
...
411 тестов: формат строк, астрономия против независимо известных значений,
...
- **Взаимные явления спутников Юпитера** считаются с точечным Солнцем: у
  настоящего затмения есть полутеневая фаза, поэтому оно начинается чуть
  раньше расчётного и заканчивается чуть позже. Сами явления бывают только
  вблизи равноденствия Юпитера — раз примерно в шесть лет.
