...
Каталоги звёзд и туманностей, элементы комет, ленты покрытий, TLE станций,
ответы Horizons, CNEOS и Launch Library скачиваются сами при первом расчёте и
кэшируются в `data/cache/`. Первый прогон занимает 10–15 минут, последующие —
3–5.
...
Рядом с exe должен лежать каталог `data/` — эфемериды и кэш каталогов не
вшиваются в сборку, чтобы обновление каталога не требовало пересборки.
