...
2. **Сближения с Землёй.** Живут в отдельном модуле `close_approaches`: у них
   свой источник (CNEOS), свои пороги значимости и свой расчёт наблюдаемости.
   Здесь оставлены только тонкие обёртки для обратной совместимости.
"""
...
def diameter_km(h_magnitude: float, albedo: float = cfg.NEO_ALBEDO) -> float:
    """Оценка диаметра по абсолютной величине H."""
    return close_approaches.diameter_km(h_magnitude, albedo)


def close_approaches_events(start: dt.datetime, end: dt.datetime,
                            limit: int | None = None) -> list[Event]:
    """Сближения NEO. Логика вынесена в `events.close_approaches`."""
    return close_approaches.build(start, end, limit)


def all_events(start: dt.datetime, end: dt.datetime) -> list[Event]:
    return close_approaches_events(start, end) + approaches(start, end)
