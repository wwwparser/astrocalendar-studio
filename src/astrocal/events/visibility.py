...
# Внутренние планеты уходят в лучи Солнца и возвращаются: у них осмысленны все
# четыре границы — начало и окончание и утренней, и вечерней видимости.
#
# У внешней планеты цикл другой: она появляется на утреннем небе после
# соединения с Солнцем, постепенно переходит к наблюдению всю ночь, потом к
# вечернему небу и уходит в лучи. Утренняя видимость у неё не «заканчивается»,
# а переходит в ночную — и писать «окончание утренней видимости Сатурна»
# неверно по смыслу. Поэтому для внешних планет публикуются только две
# границы: появление утром и уход с вечернего неба.
INNER = ("mercury", "venus")
OUTER_SLOTS = {("morning", "Начало"), ("evening", "Окончание")}
        ...
        for slot, slot_ru in (("morning", "утренней"), ("evening", "вечерней")):
            for first, last in _series_edges(days, slot):
                for day, kind in ((first, "Начало"), (last, "Окончание")):
                    if name not in INNER and (slot, kind) not in OUTER_SLOTS:
                        # переход утро→ночь→вечер у внешней планеты событием
                        # не является
                        continue
                    ...
                    out.append(Event(
                        ...
                        confidence="средняя",
                        rank="interesting",
                        computed=(f"планета {'внутренняя' if name in INNER else 'внешняя'}; "
                                  f"критерий: в момент, когда Солнце над Москвой "
