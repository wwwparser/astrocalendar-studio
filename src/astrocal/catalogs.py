...
# Русские названия самых известных объектов глубокого космоса. В OpenNGC
# собственные имена английские («Beehive»), и в русском календаре они выглядят
# инородно: читателю понятнее «Ясли». Список намеренно короткий — только то,
# что действительно называют по-русски.
DSO_NAMES_RU = {
    "M1": "Крабовидная туманность", "M3": "", "M4": "", "M6": "Бабочка",
    "M7": "скопление Птолемея", "M8": "Лагуна", "M11": "Дикие утки",
    "M13": "скопление в Геркулесе", "M16": "Орёл", "M17": "Омега",
    "M20": "Трифид", "M27": "Гантель", "M31": "Туманность Андромеды",
    "M33": "Туманность Треугольника", "M42": "Туманность Ориона",
    "M44": "Ясли", "M45": "Плеяды", "M51": "Водоворот", "M57": "Кольцо",
    "M64": "Чёрный глаз", "M76": "Малая Гантель", "M81": "галактика Боде",
    "M82": "Сигара", "M97": "Сова", "M101": "Вертушка",
    "M104": "Сомбреро",
    "NGC 869": "Хи Персея", "NGC 884": "Аш Персея",
    "NGC 7000": "Северная Америка", "NGC 253": "Серебряный доллар",
    "NGC 2237": "Розетка", "NGC 6543": "Кошачий глаз",
    "NGC 6960": "Ведьмина метла", "NGC 6992": "Вуаль",
    "IC 434": "Конская Голова",
}


def dso_common_name(messier: str | None, name: str | None,
                    english: str | None = None) -> str:
    """Название объекта для строки календаря, в кавычках-ёлочках.

    Русское имя, если оно известно; иначе английское из каталога; иначе пусто.
    """
    for key in (messier, name):
        if key and key in DSO_NAMES_RU:
            russian = DSO_NAMES_RU[key]
            return f"«{russian}»" if russian else ""
    english = (english or "").split(",")[0].strip()
    return f"«{english}»" if english else ""
